...
/*
 * Class RegistryEntry
 * Cette classe est responsable de la gestion des entrées dans la base de données.
 * Elle contient des méthodes pour se connecter à la base de données et pour insérer des données dans différentes tables.
 *
 * Les méthodes de cette classe utilisent PDO pour interagir avec la base de données PostgreSQL.
 * Elles gèrent également les exceptions pour s'assurer que les erreurs de connexion ou d'exécution des requêtes sont correctement traitées.
 *
 * Les méthodes de cette classe sont statiques, ce qui signifie qu'elles peuvent être appelées sans créer une instance de la classe.
 * Elles sont utilisées pour insérer des données dans les tables Components, Tutorials, Post, Reponse et Users.
 *
 * Les méthodes de cette classe sont :
 * - connectSQL() : Établit une connexion à la base de données PostgreSQL.
 * - buildComponentToBDD(Component $component) : Insère un composant dans la table Components.
 * - buildTutorialToBDD(Tutorial $tutorial) : Insère un tutoriel dans la table Tutorials.
 * - buildPostToBDD(Post $post) : Insère un post dans la table Post.
 * - buildReponseToBDD(Reponse $reponse) : Insère une réponse dans la table Reponse.
 * - buildUserToBDD(User $user) : Insère un utilisateur dans la table Users.
 * - getPosts() : Récupère tous les posts de la table Post.
 * - getTutorials() : Récupère tous les tutoriels de la table Tutorials.
 * - getComponents() : Récupère tous les composants de la table Components.
 * - getUsers() : Récupère tous les utilisateurs de la table Users.
 * - getUserById(int $uno) : Récupère un utilisateur par son ID de la table Users.
 * - getComponentsByTutorialNameAndLoader(int $loader, string $name) : Récupère les composants par nom de tutoriel et ID de chargeur.
 * - getBaliseFromCno(string $cno, string $description, string $code, int $lno) : Génère une balise HTML à partir des informations d'un composant.
 * - isUniqueUser(string $name, string $email) : Vérifie si un utilisateur est unique par nom et email.
 */
class RegistryEntry
{
